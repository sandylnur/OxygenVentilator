...
void loop(){
    if (ventEnabled == 0) {                                  //only execute while the ventilator is running
        counterStart = millis();                            //Start counter
        counterreset = true;                                //Counter has been reset

        while (limitSwitch == 0) {   //While the limitSwitch is Open, piston is moving
            timeMoving = millis() - counterStart;           //Measure Moving Time and compare to 6000ms
            counterreset = false;                           //Counter has been used for one check
                ...
                if (ventEnabled == 1) {                      //Perform periodic check of vent status and reset if turned off
                    break;
                }
        }

        while (limitSwitch == 1 && counterreset == true) {    //While the limitSwitch is Closed, piston is at rest
